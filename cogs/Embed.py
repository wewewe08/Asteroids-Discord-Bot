...
class Embed(commands.Cog):
    ...
    async def makeEmbed(self,ctx, grid, url, points, missiles):
        ...
        embed.add_field(name = f"🥉POINTS: {points}    📦MISSILES: {missiles}", value = grid, inline = True)
        embed.set_footer(text= f"{ctx.author.name}'s game",icon_url=url)
        return embed

    async def resetEmbed(self, ctx, moveGrid, image_url, points, missiles): #update the grid embed once the player reacts
        ...
        if gameGrid is not None:
            ...
            if points >= 5:
                new_embed.add_field(name = f"🥇POINTS: {points}    📦MISSILES: {missiles}", value = newGrid, inline = True) #
